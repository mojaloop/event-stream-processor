...
const registerEventHandler = async () => {
  try {
    let functionality = 'event'
    if (Enum.topicMap[functionality]) {
      functionality = Enum.topicMap[functionality].functionality
    }
    const EventHandler = {
      topicName: Utility.createGeneralTopicConf(functionality).topicName,
      config: Utility.getKafkaConfig(Utility.ENUMS.CONSUMER, functionality.toUpperCase())

    }
    EventHandler.config.rdkafkaConf['client.id'] = EventHandler.topicName
    await createHandler(EventHandler.topicName, EventHandler.config)
    return true
  } catch (e) {
    Logger.error(e)
    throw e
